...
<template>
  <section>
    ...
    <div
      class="flex flex-col rounded-[10px] bg-card shadow-[1px_17px_44px_0px_rgba(3,2,41,0.07)] lg:flex-row"
    >
      <!-- Left: file input + buttons -->
      <div class="flex flex-1 flex-col gap-3 p-5">
        <!-- File input row -->
        <div class="flex items-center overflow-hidden rounded-[10px] border border-border bg-background">
          <span class="flex-1 truncate px-4 py-2.5 text-sm text-foreground/20">
            {{ selectedFile ? selectedFile.name : filenamePlaceholder }}
          </span>
          <button
            class="shrink-0 border-l border-border bg-[#E9E5E5] px-5 py-2.5 text-sm font-normal text-[#626262] transition-colors hover:bg-muted"
        ...
        <button
          ...
          class="mx-auto flex w-fit items-center gap-2 rounded-[10px] bg-primary px-6 py-2.5 text-sm font-normal text-primary-foreground transition-opacity hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50"
          @click="upload"
        >
          <UIcon
